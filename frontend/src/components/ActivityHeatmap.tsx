  ...
  if (loading) {
    return (
      <div className="bg-gray-800 rounded-xl border border-gray-700 p-4">
        <div className="h-[180px] flex items-center justify-center text-gray-500">Loading...</div>
      </div>
    )
  }
  ...
  return (
    <div className="bg-gray-800 rounded-xl border border-gray-700 p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-gray-300">Activité par Heure (30 derniers jours)</h3>
      ...
      {tooltip && (
        <div className="mt-2 text-xs text-gray-400 text-center">
          {DAYS[tooltip.day]} {tooltip.hour}h–{tooltip.hour + 1}h : <span className="text-white font-medium">{tooltip.count} événements</span>
        </div>
      )}
    </div>
  )
